...
public class CareboLbsBaseView extends View {
  ...
  public CareboLbsBaseView(Context context, AttributeSet attrs) {
    super(context, attrs);
    //第一步，初始化对象
    linePaint = new Paint();
  ...
  public CareboLbsBaseView(Context context) {
    super(context);
  }
    ...
    for (int i = 0; i < 10; i++) {
      ...
      if (i == 9) {
          ...
          canvas.drawRect(width_x_start, d_rect_start_h, width_x_end*devider_percent, d_rect_end_h,
              mRectPaint);
        canvas.drawText(devider_limit_num+"", width_x_end*devider_percent+10, d_rect_start_h+Arith.mul(height+"","0.2"), mTextBigPaint);
      }
