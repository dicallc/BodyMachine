...
public class CareboLbsFatView extends View {
  ...
  public CareboLbsFatView(Context context, AttributeSet attrs) {
    super(context, attrs);
    //第一步，初始化对象
    linePaint = new Paint();
  ...
  public CareboLbsFatView(Context context) {
    super(context);
  }
  ...
  private void drawXY(Canvas canvas) {
    ...
    //一块多长
    float devider_w1 = BigDecimalUtils.div(width_x_end + "", 11 + "");
    float d_line_end_y = height - (width_y_start + BigDecimalUtils.div(height + "", "6"));
    //第一条
    float one_line_scale = 2;
    canvas.drawLine(devider_w1 * one_line_scale, height, devider_w1 * one_line_scale, d_line_end_y,
        linePaint);
    //第二条
    float two_line_scale = 4;
    canvas.drawLine(devider_w1 * two_line_scale, height, devider_w1 * two_line_scale, d_line_end_y,
        linePaint);
    //第三条
    float three_line_scale = 5;
    canvas.drawLine(devider_w1 * three_line_scale, height, devider_w1 * three_line_scale,
        d_line_end_y, linePaint);
    //第四条
    float four_line_scale = 8;
    canvas.drawLine(devider_w1 * four_line_scale, height, devider_w1 * four_line_scale,
        d_line_end_y, linePaint);
    //第5条
    //float five_line_scale = 11;
    //canvas.drawLine(devider_w1 * five_line_scale, height, devider_w1 * five_line_scale,
    //    d_line_end_y, linePaint);
    //
    if (null != arrDeviderText) {
      ...
      canvas.drawText(arrDeviderText.get(1) + "", devider_w1 * two_line_scale - 5, d_num_end_y,
          mTextPaint);
      canvas.drawText(arrDeviderText.get(2) + "", devider_w1 * three_line_scale - 5, d_num_end_y,
          mTextPaint);
      canvas.drawText(arrDeviderText.get(3) + "", devider_w1 * four_line_scale - 5, d_num_end_y,
          mTextPaint);
      //canvas.drawText(arrDeviderText.get(4) + "", devider_w1 * five_line_scale - 5, d_num_end_y,
      //    mTextPaint);
    }
    ...
    canvas.drawRect(width_x_start, d_rect_start_h, width_x_end * devider_percent, d_rect_end_h,
        mRectPaint);
    canvas.drawText(devider_limit_num + "", width_x_end * devider_percent + 10,
