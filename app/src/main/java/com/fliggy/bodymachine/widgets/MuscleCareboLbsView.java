...
public class MuscleCareboLbsView extends View {
  ...
  public MuscleCareboLbsView(Context context, AttributeSet attrs) {
    super(context, attrs);
    //第一步，初始化对象
    linePaint = new Paint();
  ...
  public MuscleCareboLbsView(Context context) {
    super(context);
  }
  ...
  private void drawXY(Canvas canvas) {
    ...
    for (int i = 0; i < 10; i++) {
      float devider_w1 = PecentToNum(i);
      ...
      if (i == 9) {
        //X Y X Y
        float mPecentToNum = PecentToNum(devider_percent*10);
        canvas.drawRect(width_x_start, d_rect_start_h, mPecentToNum, d_rect_end_h,
              mRectPaint);
        canvas.drawText(devider_limit_num+"", mPecentToNum+10, d_rect_start_h+Arith.mul(height+"","0.2"), mTextBigPaint);
      }

    }
  }

  private float PecentToNum(float mI) {
    return width_x_start + width_x_end * ((1 + (mI * 2)) / 19f);
  }
